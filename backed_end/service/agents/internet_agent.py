...
from backed_end.config.api_key import OPEN_API_KEY, TAVILY_API
...
async def get_internet_agent():
    ...
    #             ))
    tools.append(TavilySearch(tavily_api_key=TAVILY_API,max_results=10))
    ...
    internet_agent = create_react_agent(
        ...
        prompt= "you are a helpful assistant that can search the internet and use browser tools to find information."
                "you can use TavilySearch to search the internet and use browser tools to find information."
                "if you get a url from TavilySearch, you must use browser tools to visit the url and get the specific information.",
        tools=tools,
        name="internet_agent",
    )
